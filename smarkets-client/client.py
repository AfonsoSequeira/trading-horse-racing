...
class SmarketsClient:
    ...
    def get_available_events(
        ...
        states_filter = '&'.join([f'state={state}' for state in states])
        types_filter = '&'.join([f'type={type_}' for type_ in types])
        type_domains_filter = '&'.join([f'type_domain={type_domain_}' for type_domain_ in type_domains])#c
        page_filter = (
            f'?{states_filter}&{types_filter}&{type_domains_filter}&type_scope=single_event&with_new_type=true&sort=id' + 
            f'&limit={limit}&include_hidden=false&start_datetime_max={start_datetime_max}' + 
